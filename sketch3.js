...
function mousePressed() {
  if (!modoDetalle && !enAnimacion && !animacionReversa) {
    if (mouseX >= gridX && mouseX <= gridX + gridAncho && mouseY >= gridY && mouseY <= gridY + gridAlto) {
      let anchoCelda = gridAncho / 3;
      let altoCelda = gridAlto / 3;
      let col = floor((mouseX - gridX) / anchoCelda);
      let fila = floor((mouseY - gridY) / altoCelda);
    ...
    if (dragActivo) {
      for (let i = cSquares.length - 1; i >= 0; i--) {
        let sq = cSquares[i];
        let halfS = sq.sSize / 2;
        if (mouseX >= sq.x - halfS && mouseX <= sq.x + halfS && mouseY >= sq.y - halfS && mouseY <= sq.y + halfS) {
          squareDragging = sq;
          offsetXDrag = mouseX - sq.x;
          offsetYDrag = mouseY - sq.y;
          break;
        }
      }
    } else if (dragActivo3) {
      for (let i = cSquares3.length - 1; i >= 0; i--) {
        ...
        if (mouseX >= sq.x - halfS && mouseX <= sq.x + halfS && mouseY >= sq.y - halfS && mouseY <= sq.y + halfS) {
          squareDragging3 = sq;
          offsetXDrag3 = mouseX - sq.x;
          offsetYDrag3 = mouseY - sq.y;
          break;
        }
      }
    } else if (dragActivo2) {
      for (let i = cSquares2.length - 1; i >= 0; i--) {
        ...
        if (mouseX >= sq.x - halfS && mouseX <= sq.x + halfS && mouseY >= sq.y - halfS && mouseY <= sq.y + halfS) {
          squareDragging2 = sq;
          offsetXDrag2 = mouseX - sq.x;
          offsetYDrag2 = mouseY - sq.y;
          break;
        }
      }
      ...
      for (let i = 0; i < 5; i++) {
        ...
        if (c && dist(mouseX, mouseY, c.x, c.y) < c.r) {
...
function mouseDragged() {
  if (dragActivo && squareDragging) {
    let targetX = mouseX - offsetXDrag;
    let targetY = mouseY - offsetYDrag;
    ...
    let targetX = mouseX - offsetXDrag3;
    let targetY = mouseY - offsetYDrag3;
    ...
    let targetX = mouseX - offsetXDrag2;
    let targetY = mouseY - offsetYDrag2;
...
// --- NUEVAS FUNCIONES PARA ZOOM (MOUSE Y TÁCTIL) ---
function mouseWheel(event) {
  if (modoDetalle && !enAnimacion && !animacionReversa && columnaSeleccionada === 1 && filaSeleccionada === 0) {
    // Si la rueda se mueve hacia adelante (positivo), se achica. Hacia atrás (negativo), se agranda.
    if (event.delta > 0) {
...
function touchMoved() {
  if (modoDetalle && !enAnimacion && !animacionReversa && columnaSeleccionada === 1 && filaSeleccionada === 0) {
    if (touches.length === 2 && initialPinchDist !== null) {
      ...
      // Zoom-in (acercar con los dedos -> distancia aumenta) -> el triángulo se achica.
      // Zoom-out (alejar con los dedos -> distancia disminuye) -> el triángulo se agranda.
      if (abs(diff) > 2) {
        if (diff > 0) {
