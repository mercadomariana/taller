...
// --- VARIABLES DE ZOOM INDIVIDUAL PARA LOS TRIÁNGULOS PEQUEÑOS ---
...
function draw() {
  ...
  // Actualizar interpolación con delay para el zoom de los triángulos
...
function dibujarFigura(col, fila, x, y, tamano) {
  ...
  if (tipo === 0) {
    ...
    let factorSup = (modoDetalle && !enAnimacion && !animacionReversa && esTerceraColumnaTerceraFila) ? trianguloSupFactor : 1.0;
    ...
    let factorIzq = (modoDetalle && !enAnimacion && !animacionReversa && esTerceraColumnaTerceraFila) ? trianguloIzqFactor : 1.0;
    ...
    let factorDer = (modoDetalle && !enAnimacion && !animacionReversa && esTerceraColumnaTerceraFila) ? trianguloDerFactor : 1.0;
...
function touchStarted() {
  if (touches.length === 1) {
    ...
    if (modoDetalle && !enAnimacion && !animacionReversa && columnaSeleccionada === 2 && filaSeleccionada === 2) {
...
function touchMoved() {
  if (modoDetalle && !enAnimacion && !animacionReversa && columnaSeleccionada === 2 && filaSeleccionada === 2) {
    if (touches.length === 2 && initialPinchDist !== null) {
      ...
      if (abs(diff) > 2) {
        // Zoom-in (diff > 0) agranda los otros 3%, Zoom-out (diff < 0) achica los otros 3%
...
function mouseWheel(event) {
  if (modoDetalle && !enAnimacion && !animacionReversa && columnaSeleccionada === 2 && filaSeleccionada === 2) {
    ...
    // event.delta < 0 (rueda hacia adelante) agranda los otros, event.delta > 0 (rueda hacia atrás) achica los otros
