...
// --- VARIABLES DE ZOOM INDIVIDUAL PARA LOS TRIÁNGULOS PEQUEÑOS (Tercer columna, tercer fila) ---
let trianguloSupFactor = 1.0;
let trianguloIzqFactor = 1.0;
let trianguloDerFactor = 1.0;
let initialPinchDist = null;
let trianguloSeleccionadoZoom = -1; // 0: Superior, 1: Izquierdo, 2: Derecho

// Variables para el delay del zoom con animación suave (lerp)
let trianguloSupTargetFactor = 1.0;
let trianguloIzqTargetFactor = 1.0;
let trianguloDerTargetFactor = 1.0;
let delayZoomTimer = 0;
let delayZoomActivo = false;
let delayZoomPendienteDelta = 0;

// --- VARIABLES DE ZOOM INDIVIDUAL PARA LOS TRIÁNGULOS PEQUEÑOS (Primer columna, tercer fila) ---
let triCol0Row2SupFactor = 1.0;
let triCol0Row2IzqFactor = 1.0;
let triCol0Row2DerFactor = 1.0;
let initialPinchDistCol0Row2 = null;
let trianguloCol0Row2SeleccionadoZoom = -1;

let triCol0Row2SupTargetFactor = 1.0;
let triCol0Row2IzqTargetFactor = 1.0;
let triCol0Row2DerTargetFactor = 1.0;
let delayZoomCol0Row2Timer = 0;
let delayZoomCol0Row2Activo = false;
let delayZoomCol0Row2PendienteDelta = 0;

// --- VARIABLES DE ZOOM INDIVIDUAL PARA LOS TRIÁNGULOS PEQUEÑOS (Segunda columna, primer fila) ---
let triCol1Row0SupFactor = 1.0;
let triCol1Row0IzqFactor = 1.0;
let triCol1Row0DerFactor = 1.0;
let initialPinchDistCol1Row0 = null;
let trianguloCol1Row0SeleccionadoZoom = -1;

let triCol1Row0SupTargetFactor = 1.0;
let triCol1Row0IzqTargetFactor = 1.0;
let triCol1Row0DerTargetFactor = 1.0;
let delayZoomCol1Row0Timer = 0;
let delayZoomCol1Row0Activo = false;
let delayZoomCol1Row0PendienteDelta = 0;
...
function draw() {
  ...
  // Actualizar interpolación con delay para el zoom de los triángulos en (2, 2)
  if (delayZoomActivo && millis() - delayZoomTimer >= 500) {
    if (trianguloSeleccionadoZoom === 0) {
      trianguloSupTargetFactor = constrain(trianguloSupTargetFactor + delayZoomPendienteDelta, 0.2, 3.0);
    } else if (trianguloSeleccionadoZoom === 1) {
      trianguloIzqTargetFactor = constrain(trianguloIzqTargetFactor + delayZoomPendienteDelta, 0.2, 3.0);
    } else if (trianguloSeleccionadoZoom === 2) {
      trianguloDerTargetFactor = constrain(trianguloDerTargetFactor + delayZoomPendienteDelta, 0.2, 3.0);
    }
    delayZoomActivo = false;
  }

  trianguloSupFactor = lerp(trianguloSupFactor, trianguloSupTargetFactor, 0.1);
  trianguloIzqFactor = lerp(trianguloIzqFactor, trianguloIzqTargetFactor, 0.1);
  trianguloDerFactor = lerp(trianguloDerFactor, trianguloDerTargetFactor, 0.1);

  // Actualizar interpolación con delay para el zoom de los triángulos en (0, 2)
  if (delayZoomCol0Row2Activo && millis() - delayZoomCol0Row2Timer >= 1300) {
    if (trianguloCol0Row2SeleccionadoZoom === 0) {
      triCol0Row2SupTargetFactor = constrain(triCol0Row2SupTargetFactor + delayZoomCol0Row2PendienteDelta, 0.2, 3.0);
    } else if (trianguloCol0Row2SeleccionadoZoom === 1) {
      triCol0Row2IzqTargetFactor = constrain(triCol0Row2IzqTargetFactor + delayZoomCol0Row2PendienteDelta, 0.2, 3.0);
    } else if (trianguloCol0Row2SeleccionadoZoom === 2) {
      triCol0Row2DerTargetFactor = constrain(triCol0Row2DerTargetFactor + delayZoomCol0Row2PendienteDelta, 0.2, 3.0);
    }
    delayZoomCol0Row2Activo = false;
  }

  triCol0Row2SupFactor = lerp(triCol0Row2SupFactor, triCol0Row2SupTargetFactor, 0.1);
  triCol0Row2IzqFactor = lerp(triCol0Row2IzqFactor, triCol0Row2IzqTargetFactor, 0.1);
  triCol0Row2DerFactor = lerp(triCol0Row2DerFactor, triCol0Row2DerTargetFactor, 0.1);

  // Actualizar interpolación para el zoom de los triángulos en (1, 0)
  triCol1Row0SupFactor = lerp(triCol1Row0SupFactor, triCol1Row0SupTargetFactor, 0.1);
  triCol1Row0IzqFactor = lerp(triCol1Row0IzqFactor, triCol1Row0IzqTargetFactor, 0.1);
  triCol1Row0DerFactor = lerp(triCol1Row0DerFactor, triCol1Row0DerTargetFactor, 0.1);
...
function dibujarFigura(col, fila, x, y, tamano) {
  ...
  if (tipo === 0) {
    ...
    // El triángulo contenedor negro mantiene siempre un tamaño fijo (factor 1.0)
    let r = tamano / 1.6;
    let offsetY = tamano * 0.15; 
    let baseY = y + offsetY; 
    ...
    let ladoBasePequeno = r * 0.35; 
    let hPequeno = ladoBasePequeno * 0.866; 

    let esTerceraColumnaTerceraFila = (col === 2 && fila === 2);
    let esPrimeraColumnaTerceraFila = (col === 0 && fila === 2);
    let esSegundaColumnaPrimeraFila = (col === 1 && fila === 0);

    // 1. Triángulo superior (Factor individual)
    let centroArribaX = vArribaX;
    let centroArribaY = vArribaY + hPequeno * 2;
    let factorSup = 1.0;
    if (modoDetalle && !enAnimacion && !animacionReversa) {
      if (esTerceraColumnaTerceraFila) factorSup = trianguloSupFactor;
      else if (esPrimeraColumnaTerceraFila) factorSup = triCol0Row2SupFactor;
      else if (esSegundaColumnaPrimeraFila) factorSup = triCol1Row0SupFactor;
    }
    let ladoSup = ladoBasePequeno * factorSup;
    dibujarTrianguloEquilatero(centroArribaX, centroArribaY, ladoSup, 2);

    // 2. Triángulo inferior izquierdo (Factor individual)
    let centroIzqX = vIzqX + ladoBasePequeno * 1.5;
    let centroIzqY = vIzqY - hPequeno * 1;
    let factorIzq = 1.0;
    if (modoDetalle && !enAnimacion && !animacionReversa) {
      if (esTerceraColumnaTerceraFila) factorIzq = trianguloIzqFactor;
      else if (esPrimeraColumnaTerceraFila) factorIzq = triCol0Row2IzqFactor;
      else if (esSegundaColumnaPrimeraFila) factorIzq = triCol1Row0IzqFactor;
    }
    let ladoIzq = ladoBasePequeno * factorIzq;
    dibujarTrianguloEquilatero(centroIzqX, centroIzqY, ladoIzq, 1);

    // 3. Triángulo inferior derecho (Factor individual)
    let centroDerX = vDerX - ladoBasePequeno * 1.5;
    let centroDerY = vDerY - hPequeno * 1;
    let factorDer = 1.0;
    if (modoDetalle && !enAnimacion && !animacionReversa) {
      if (esTerceraColumnaTerceraFila) factorDer = trianguloDerFactor;
      else if (esPrimeraColumnaTerceraFila) factorDer = triCol0Row2DerFactor;
      else if (esSegundaColumnaPrimeraFila) factorDer = triCol1Row0DerFactor;
    }
    let ladoDer = ladoBasePequeno * factorDer;
    dibujarTrianguloEquilatero(centroDerX, centroDerY, ladoDer, 0);
  }
}

function obtenerTrianguloCercano(px, py) {
  let centroX = width / 2;
  let centroY = height / 2;
  let tamano = min(width, height) * 0.88;
  let r = tamano / 1.6;
  let offsetY = tamano * 0.15;
  let baseY = centroY + offsetY;

  let vArribaX = centroX;
  let vArribaY = baseY - r;
  let vIzqX = centroX - r * 0.866;
  let vIzqY = baseY + r * 0.5;
  let vDerX = centroX + r * 0.866;
  let vDerY = baseY + r * 0.5;
  ...
  let centros = [
    { id: 0, x: vArribaX, y: vArribaY + hPequeno * 2 }, // Superior
    { id: 1, x: vIzqX + ladoBasePequeno * 1.5, y: vIzqY - hPequeno * 1 }, // Izquierdo
    { id: 2, x: vDerX - ladoBasePequeno * 1.5, y: vDerY - hPequeno * 1 }  // Derecho
  ];

  let mejorId = 0;
  let menorDist = Infinity;
  for (let c of centros) {
    let d = dist(px, py, c.x, c.y);
    if (d < menorDist) {
      menorDist = d;
      mejorId = c.id;
    }
...
function aplicarZoomTrianguloInverso(accionZoom) {
  // accionZoom: 'zoom-in' (crecer los otros 3%) o 'zoom-out' (achicar los otros 3%)
  let delta = (accionZoom === 'zoom-in') ? 0.03 : -0.03;

  if (trianguloSeleccionadoZoom === 0) {
    // El superior queda igual, los otros dos (izquierdo y derecho) cambian un 3%
    trianguloIzqTargetFactor = constrain(trianguloIzqTargetFactor + delta, 0.2, 3.0);
    trianguloDerTargetFactor = constrain(trianguloDerTargetFactor + delta, 0.2, 3.0);
  } else if (trianguloSeleccionadoZoom === 1) {
    // El izquierdo queda igual, los otros dos (superior y derecho) cambian un 3%
    trianguloSupTargetFactor = constrain(trianguloSupTargetFactor + delta, 0.2, 3.0);
    trianguloDerTargetFactor = constrain(trianguloDerTargetFactor + delta, 0.2, 3.0);
  } else if (trianguloSeleccionadoZoom === 2) {
    // El derecho queda igual, los otros dos (superior e izquierdo) cambian un 3%
    trianguloSupTargetFactor = constrain(trianguloSupTargetFactor + delta, 0.2, 3.0);
    trianguloIzqTargetFactor = constrain(trianguloIzqTargetFactor + delta, 0.2, 3.0);
  }
}

function aplicarZoomCol1Row2Individual(accionZoom) {
  // Zoom-in: alejar (rueda adelante / zoom-in táctil) -> achicar triángulo
  // Zoom-out: acercar (rueda atrás / zoom-out táctil) -> agrandar triángulo
  let delta = (accionZoom === 'zoom-in') ? -0.1 : 0.1;

  if (trianguloCol1Row0SeleccionadoZoom === 0) {
    triCol1Row0SupTargetFactor = constrain(triCol1Row0SupTargetFactor + delta, 0.2, 3.0);
  } else if (trianguloCol1Row0SeleccionadoZoom === 1) {
    triCol1Row0IzqTargetFactor = constrain(triCol1Row0IzqTargetFactor + delta, 0.2, 3.0);
  } else if (trianguloCol1Row0SeleccionadoZoom === 2) {
    triCol1Row0DerTargetFactor = constrain(triCol1Row0DerTargetFactor + delta, 0.2, 3.0);
  }
}

function aplicarZoomCol0Row2Individual(accionZoom) {
  let delta = (accionZoom === 'zoom-in') ? 0.8 : -0.8;
  delayZoomCol0Row2PendienteDelta = delta;
  delayZoomCol0Row2Timer = millis();
  delayZoomCol0Row2Activo = true;
}
  ...
  if (!modoDetalle && !enAnimacion && !animacionReversa) {
    if (px >= gridX && px <= gridX + gridAncho && py >= gridY && py <= gridY + gridAlto) {
      ...
      if (col >= 0 && col < 3 && fila >= 0 && fila < 3) {
        ...
        if (col === 2 && fila === 2) {
          trianguloSupFactor = 1.0;
          trianguloIzqFactor = 1.0;
          trianguloDerFactor = 1.0;
          trianguloSupTargetFactor = 1.0;
          trianguloIzqTargetFactor = 1.0;
          trianguloDerTargetFactor = 1.0;
        } else if (col === 0 && fila === 2) {
          triCol0Row2SupFactor = 1.0;
          triCol0Row2IzqFactor = 1.0;
          triCol0Row2DerFactor = 1.0;
          triCol0Row2SupTargetFactor = 1.0;
          triCol0Row2IzqTargetFactor = 1.0;
          triCol0Row2DerTargetFactor = 1.0;
        } else if (col === 1 && fila === 0) {
          triCol1Row0SupFactor = 1.0;
          triCol1Row0IzqFactor = 1.0;
          triCol1Row0DerFactor = 1.0;
          triCol1Row0SupTargetFactor = 1.0;
          triCol1Row0IzqTargetFactor = 1.0;
          triCol1Row0DerTargetFactor = 1.0;
        }
    ...
    if (columnaSeleccionada === 2 && filaSeleccionada === 2) {
      trianguloSeleccionadoZoom = obtenerTrianguloCercano(px, py);
    } else if (columnaSeleccionada === 0 && filaSeleccionada === 2) {
      trianguloCol0Row2SeleccionadoZoom = obtenerTrianguloCercano(px, py);
    } else if (columnaSeleccionada === 1 && filaSeleccionada === 0) {
      trianguloCol1Row0SeleccionadoZoom = obtenerTrianguloCercano(px, py);
    }
...
function touchStarted() {
  if (touches.length === 1) {
    ...
    initialPinchDistCol0Row2 = initialPinchDist;
    initialPinchDistCol1Row0 = initialPinchDist;
    if (modoDetalle && !enAnimacion && !animacionReversa) {
      let midX = (touches[0].x + touches[1].x) / 2;
      let midY = (touches[0].y + touches[1].y) / 2;
      if (columnaSeleccionada === 2 && filaSeleccionada === 2) {
        trianguloSeleccionadoZoom = obtenerTrianguloCercano(midX, midY);
      } else if (columnaSeleccionada === 0 && filaSeleccionada === 2) {
        trianguloCol0Row2SeleccionadoZoom = obtenerTrianguloCercano(midX, midY);
      } else if (columnaSeleccionada === 1 && filaSeleccionada === 0) {
        trianguloCol1Row0SeleccionadoZoom = obtenerTrianguloCercano(midX, midY);
      }
    }
  }
  return false;
  ...
  if (modoDetalle && !enAnimacion && !animacionReversa) {
    if (columnaSeleccionada === 2 && filaSeleccionada === 2) {
      if (touches.length === 2 && initialPinchDist !== null) {
        ...
        if (abs(diff) > 2) {
          let accion = (diff > 0) ? 'zoom-in' : 'zoom-out';
          aplicarZoomTrianguloInverso(accion);
          initialPinchDist = currentPinchDist;
        }
        return false;
      }
    } else if (columnaSeleccionada === 0 && filaSeleccionada === 2) {
      if (touches.length === 2 && initialPinchDistCol0Row2 !== null) {
        let currentPinchDist = dist(touches[0].x, touches[0].y, touches[1].x, touches[1].y);
        let diff = currentPinchDist - initialPinchDistCol0Row2;
        
        if (abs(diff) > 2) {
          let accion = (diff > 0) ? 'zoom-in' : 'zoom-out';
          
          let delta = (accion === 'zoom-in') ? 0.10 : -0.10;
          delayZoomCol0Row2PendienteDelta = delta;
          delayZoomCol0Row2Timer = millis();
          delayZoomCol0Row2Activo = true;

          initialPinchDistCol0Row2 = currentPinchDist;
        }
        return false;
      }
    } else if (columnaSeleccionada === 1 && filaSeleccionada === 0) {
      if (touches.length === 2 && initialPinchDistCol1Row0 !== null) {
        let currentPinchDist = dist(touches[0].x, touches[0].y, touches[1].x, touches[1].y);
        let diff = currentPinchDist - initialPinchDistCol1Row0;
        
        if (abs(diff) > 2) {
          let accion = (diff > 0) ? 'zoom-in' : 'zoom-out';
          aplicarZoomCol1Row2Individual(accion);
          initialPinchDistCol1Row0 = currentPinchDist;
        }
        return false;
      }
    }
...
function touchEnded() {
  if (touches.length < 2) {
    ...
    initialPinchDistCol0Row2 = null;
    initialPinchDistCol1Row0 = null;
  }
  procesarAccionFin();
  return false;
}
...
function mouseWheel(event) {
  if (modoDetalle && !enAnimacion && !animacionReversa) {
    if (columnaSeleccionada === 2 && filaSeleccionada === 2) {
      trianguloSeleccionadoZoom = obtenerTrianguloCercano(mouseX, mouseY);
      let accion = (event.delta < 0) ? 'zoom-in' : 'zoom-out';
      aplicarZoomTrianguloInverso(accion);
      return false; 
    } else if (columnaSeleccionada === 0 && filaSeleccionada === 2) {
      trianguloCol0Row2SeleccionadoZoom = obtenerTrianguloCercano(mouseX, mouseY);
      let accion = (event.delta < 0) ? 'zoom-in' : 'zoom-out';
      ...
      trianguloCol1Row0SeleccionadoZoom = obtenerTrianguloCercano(mouseX, mouseY);
      let accion = (event.delta < 0) ? 'zoom-in' : 'zoom-out';
      aplicarZoomCol1Row2Individual(accion);
      return false;
    }
